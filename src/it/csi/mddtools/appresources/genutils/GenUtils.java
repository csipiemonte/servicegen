...
public class GenUtils {
	...
	/**
	 * plugin pd ejb modalit� vecchia (package ejb)
	 */
	public final static String ANNOTATION_VALUE_PDPLUGIN_EJB1 = "ejbpa-1";
	
	/**
	 * plugin pd ejb modalit� nuova (package ejb20)
	 */
	 ...
	 * es: iridev2-pep-intf-2.0.0 (non si deve aggiungere l'estensione che � automaticamente .jar)
	 */
		...
		while (it_si.hasNext()) {
			...
			if (!visited_si.contains(serviceImpl)){
				if (serviceImpl.getServiceComponent()!= null && serviceImpl.getServiceComponent() instanceof ResourceBasedSC){
					// TODO pezza tmeporanea per risolvere il fatto che la reference "resourceSet" non � 
